...
def convert_obb(
    ...
    Convert oriented bounding boxes (OBBs) between:
      ...
      - 'cxcywhr' (center x, center y, width, height, angle in radian)
    ...
    in_form, in_type, in_mult = format_in.split('_')
    out_form, out_type, out_mult = format_out.split('_')
    forms = {'cxcywha','cxcywhr','pts','flat'}
    types = {'array','list'}
    mults = {'single','multi'}
    ...
    for cx, cy, w, h, angle in obb_list:
        ...
        elif out_form == 'cxcywhr':
            out_arr = np.array([cx, cy, w, h, angle * np.pi / 180], dtype=float)        
        else:
...
def convert_anything_with_transform( # Warning TODO VERIFY
    input: Union[List, np.ndarray],
    transform: Union[Callable, Tuple[str, str]],
    object_in: AnyObject,
    format_in: AnyFormat,
    format_out: AnyFormat = None,
    inverted: bool = False,
) -> Union[List, np.ndarray]:
    """
    Convert objects (segmentation, bounding box, or point) between pixel and EPSG coordinate spaces using linear interpolation.
    ...
    if isinstance(transform, Tuple):
        ...
        transformer = Transformer.from_crs(transform[0], transform[1], always_xy=True)
...
def get_anything_area(
    obj: Union[List, np.ndarray],
    object_in: Literal['segmentation', 'bbox', 'obb', 'point'],
    format_in: Union[SegmentationFormat, BoxFormat, OrientedBoxFormat, PointFormat]
) -> float:
    """
    Compute the area of any annotation (segmentation, bbox, obb, or point) by first
    converting it into a polygonal segmentation and then applying the shoelace formula.

    Returns:
        The total area (float). For a point, returns 0.0.
    """
    seg_polys = convert_anything_to_segmentation(
        obj,
        object_in=object_in,
        format_in=format_in,
        format_out='pts_array_multi'
    ...
    for poly in seg_polys:
        # cv2.contourArea expects float32 or float64
        total_area += cv2.contourArea(poly.astype(np.float32))

    return float(total_area)


def get_anything_contour(
    obj: Union[List, np.ndarray],
    object_in: Literal['segmentation', 'bbox', 'obb', 'point'],
    format_in: Union[SegmentationFormat, BoxFormat, OrientedBoxFormat, PointFormat]
) -> float:
    ...
    for poly in seg_polys:
        # cv2.contourArea expects float32 or float64
        total_contour += cv2.arcLength(poly.astype(np.float32), True)

    return float(total_contour)
...
def write_svg(
    ...
    for ann, color in zip(objects, colors):
        ...
        if not isinstance(color, str):
            color = IDX_TO_COLOR_STR[int(color)]
