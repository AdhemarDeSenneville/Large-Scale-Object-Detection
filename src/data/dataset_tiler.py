...
from .utils_annotation import convert_segmentation_to_bbox, convert_anything_with_lerp, convert_anything_to_segmentation, convert_anything_with_transform
from .utils_annotation import get_anything_area, IDX_TO_COLOR_STR, IDX_TO_CLASS, write_svg
from .utils_image import convert_image
import time

CRS_FOLIUM = CRS.from_epsg(4326)
...
class TileSlidingGenerator():
    ...
    def __init__(
        ...
        if "pixel_geometry" in zones_of_tiles.columns:
            self.zones_with_pixels = (
                zones_of_tiles
                .drop(columns=["geometry"], errors="ignore")
                .set_geometry("pixel_geometry", inplace=False)
                .to_crs(epsg)
            )
        else:
            self.zones_with_pixels = zones_of_tiles.to_crs(epsg)
        
        if ("queryable_geometry" in zones_of_tiles.columns and
            zones_of_tiles["queryable_geometry"].notnull().any()):
            self.zones_queryable = (
                zones_of_tiles
                .drop(columns=["geometry"])
                .set_geometry("queryable_geometry", inplace=False)
                .to_crs(epsg)
            )
        else:
            self.zones_queryable = None
        ...
        if folder_results is None:
            self.folder_results = os.path.join(folder, 'results')
        else:
    ...
    def get_map_base(self, m):
        ...
        m_satellite = folium.TileLayer(
            tiles="https://mt1.google.com/vt/lyrs=s&x={x}&y={y}&z={z}",
            attr="Google",
            name="Google Satellite (Overlay)",  # Display name in control
            overlay=True,
            control=True
        )

        wms_layer = folium.WmsTileLayer(
            url="https://service.pdok.nl/kadaster/basisvoorziening-3d/terugmeldingen/wms/v1_0",
            layers="kad3dbasisvoorzieningterugmeldingen",
            fmt="image/png",
            transparent=True,
            version="1.3.0",
            attr="PDOK",
            name="3D Terugmeldingen",
            overlay=True,
            control=True,
            show=True
        )
        ...
        # Define CRS
        m_geo_zone = folium.FeatureGroup(name="Zone Mapped", show=True)

        bounds = self.zones_to_map.to_crs(CRS_FOLIUM).total_bounds  # (minx, miny, maxx, maxy)
        m.fit_bounds([[bounds[1], bounds[0]], [bounds[3], bounds[2]]]) 
        
        # Add geographique zone border to Folium map
        folium.GeoJson(
            self.zones_to_map.to_crs(CRS_FOLIUM),
...
class TileSlidingGeneratorShapes(TileSlidingGenerator):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.allow_out_of_pixel_tiles = kwargs.pop('allow_out_of_pixel_tiles', True) # TODO
    ...
    def construct_tiling(self):
        ...
        for idx, zone_with_pixels in tqdm(enumerate(self.zones_with_pixels.itertuples()), total=len(self.zones_with_pixels), desc="Zones", position=0):


            # Skip entire zone if it doesn't overlap any mapping zone
            if not self.zones_to_map.geometry.intersects(zone_with_pixels.geometry).any():
                continue
            ...
            if self.zones_queryable:
                zone_queryable = self.zones_queryable.iloc[idx]
            
            xs = np.arange(min_x, max_x, self.hop_size_metre)
            ys = np.arange(min_y, max_y, self.hop_size_metre)

            for x in tqdm(xs, total=len(xs), desc="Steps", position=1, leave=False):
                for y in ys:
                    tile = box(x, y, x + self.tile_size_metre, y + self.tile_size_metre)

                    if self.zones_queryable:
                        if not zone_queryable.geometry.contains(tile):
                            continue
    ...
    def _load_all_detections(self): # TODO all results in detection folder 
        
        path_to_raw_detections = os.path.join(self.folder_results, 'raw_detections')
        raw_detections = []
        for file in os.listdir(path_to_raw_detections):
            if file.endswith(".json"):
                file_path = os.path.join(path_to_raw_detections, file)
                with open(file_path, "r") as f:
                    data = json.load(f)
                    print('File:',file_path, len(data))
                    raw_detections.extend(data)

        return raw_detections


    def _load_selected_detections(self):
        with open(os.path.join(self.folder_results, "selected_detections.json"), "r") as file:
            selected_detections = json.load(file)
        
        return selected_detections


    def _add_map_selected_detections(self, verbose = True):

        m_selected_detections = folium.FeatureGroup(name="Selected Detecions", show=True)

        selected_detections = self._load_selected_detections()

        transform = (self.epsg, CRS_FOLIUM)

        for selected_detection in tqdm(selected_detections, desc="Rendering Selected Detections", disable=~verbose):

            color = IDX_TO_COLOR_STR[1]
            score = selected_detection['global_score']
            classe = 'full site'

            area = get_anything_area(
                selected_detection["segmentation"],
                object_in = 'segmentation',
                format_in = 'flat_list_multi',
            )
            
            segmentation = convert_anything_with_transform(
                selected_detection["segmentation"],
                transform,
                object_in = 'segmentation',
                format_in = 'flat_list_multi',
                format_out = 'pts_list_single',
            )
            segmentation_latlon = [(y, x) for x, y in segmentation]
            folium.Polygon(
                locations=segmentation_latlon,
                color=color,
                fill=True,
                fill_color=color,
                fill_opacity=0.01,
                weight=2,
                tooltip=f"{classe}, LogScore: {np.log(score):.2f}, Area {area:.2f}m²"
            ).add_to(m_selected_detections)

              # --- Find top-most segment ---
            min_index, pt1 = max(enumerate(segmentation_latlon), key=lambda x: x[1][0])
            pt2 = segmentation_latlon[(min_index - 1) % len(segmentation_latlon)]
            ...
            folium.Marker(
                ...
                tooltip=f"{classe}, LogScore: {np.log(score):.2f}, Area {area:.2f}"
            ).add_to(m_selected_detections)

            for sub_detection in selected_detection['inside_list']:
                color = IDX_TO_COLOR_STR[sub_detection["category_id"]]
                score = sub_detection["score"]
                classe = IDX_TO_CLASS[sub_detection["category_id"]]

                area = get_anything_area(
                    sub_detection["segmentation"],
                    object_in = 'segmentation',
                    format_in = 'flat_list_multi',
                )
                ...
                folium.Polygon(
                    ...
                    #fill_opacity=0.1,
                    weight=2,
                    tooltip=f"{classe}, Score: {score:.2f}, Area {area:.2f} "
                ).add_to(m_selected_detections)
        
        return m_selected_detections
        

    def _add_map_raw_detections(self, verbose = True):

        m_raw_detections = folium.FeatureGroup(name="Raw Detecions", show=True)

        transform = (self.epsg, CRS_FOLIUM)

        raw_detections = self._load_all_detections()

        for raw_detection in tqdm(raw_detections, desc="Rendering Detections", disable=~verbose):

            color = IDX_TO_COLOR_STR[raw_detection["category_id"]]
            score = raw_detection["score"]
            
            segmentation = convert_anything_with_transform(
                raw_detection["segmentation"],
                transform,
                object_in = 'segmentation',
                format_in = 'flat_list_multi',
                format_out = 'pts_list_single',
            ...
            folium.Polygon(
                ...
                #fill_opacity=0.1,
                weight=2,
                tooltip=f"Score: {score:.2f}, Category ID: {raw_detection['category_id']}"
            ).add_to(m_raw_detections)

        return m_raw_detections


    def get_map(
            self, 
            add_raw_detections = False,
            add_selected_detections = False,
            verbose = True
        ):

        m_tiles = folium.FeatureGroup(name="Tiles", show=True)
        m_unions = folium.FeatureGroup(name="Union of Tiles", show=True)
        m_queryable = folium.FeatureGroup(name="Queryable", show=True)
        ...
        for i, dico in enumerate(meta_meta_data):
            ...
            # --- Bound --- #
            gdf_bounds = gpd.GeoSeries(densify_box(*dico["bounds"]), crs=self.epsg).to_crs(CRS_FOLIUM)
            bounds_latlon = [(y, x) for x, y in gdf_bounds.geometry[0].exterior.coords]
            folium.GeoJson(gdf_bounds, color="grey", weight=1, fill=False, tooltip=f"Tile {i} Bounds").add_to(m_tiles)
            #for latlon in bounds_latlon[:-1]:
            #    folium.CircleMarker(latlon, color="grey", radius=3, fill=False).add_to(m_tiles)

            # --- Centre --- #
            polygon = box(*dico["bounds"])
            center = gpd.GeoSeries([polygon.centroid], crs=self.epsg).to_crs(CRS_FOLIUM).geometry[0]
            folium.Marker(
            ...
            # --- Unions --- #
            union_geom = loads(dico["union_tiles"])
            gdf_union = gpd.GeoSeries([union_geom], crs=self.epsg).to_crs(CRS_FOLIUM)
            ...
            # --- Queryable --- #
        if self.zones_queryable is not None:
            for i, zone_queryable in enumerate(self.zones_queryable.itertuples()):
                gdf_queryable = gpd.GeoSeries([zone_queryable.geometry], crs=self.epsg).to_crs(CRS_FOLIUM)
                folium.GeoJson(gdf_queryable, color="red", weight=1, fill=False, tooltip=f"Zone Queryable").add_to(m_queryable)


        m_tiles.add_to(m)
        m_unions.add_to(m)
        m_queryable.add_to(m) # TODO Test

        if add_raw_detections:
            m_raw_detection = self._add_map_raw_detections(verbose)
            m_raw_detection.add_to(m)

        if add_selected_detections:
            m_selected_detection = self._add_map_selected_detections(verbose)
            m_selected_detection.add_to(m)
            
        super().get_map_base(m)


    def tile_iterator(self, dataset_cropper = None, start_at = -1):
    ...
    def run(
            ...
            score_threshold = 0.05, # avoid having to many detections
            resume = True,
            ):
        
        # Avoide data download errors
        from PIL import ImageFile
        ImageFile.LOAD_TRUNCATED_IMAGES = True
        
        if folder_results is None:
            folder_results = self.folder_results
        else: 
            self.folder_results = folder_results
        path_submission = os.path.join(folder_results, 'raw_detections')
        os.makedirs(path_submission, exist_ok=True)


        #─── resume logic ──── TODO TEST
        existing_files = [
            fname for fname in os.listdir(path_submission)
            if fname.startswith('predictions_') and fname.endswith('.json')
        ]
        print(existing_files)
        if existing_files:
            if not resume:
                raise RuntimeError(f"Found existing prediction files in {path_submission} but resume=False")
            # extract the largest index from filenames like "predictions_{i:09d}.json"
            max_saved_i = -1
            for fname in existing_files:
                try:
                    idx_str = fname[len('predictions_'):-len('.json')]
                    idx = int(idx_str)
                    if idx > max_saved_i:
                        max_saved_i = idx
                except ValueError:
            ...
            print(f'Resuming run at: {max_saved_i}')
        else:
            max_saved_i = -1
            print(f'Starting a new run: {max_saved_i}')
        #───────────────────
        ...
        try:
            ...
            for i, tile in enumerate(tqdm(self.tile_iterator(dataset_cropper, start_at = max_saved_i), total = self.len)):
                ...
                if False and len(annotations) != 0:
                    from PIL import Image
                    svg_folder = '/media/adhemar/LaCie1/temp/denmark_1/anns'
                    img_folder = '/media/adhemar/LaCie1/temp/denmark_1/imgs'
                    os.makedirs(svg_folder, exist_ok=True)
                    os.makedirs(img_folder, exist_ok=True)

                    # 3a) Write the SVG of all detections in this tile
                    path_to_svgs_temp = os.path.join(svg_folder, f'{i:09d}.svg')
                    write_svg(
                        annotations,
                        labels,
                        object_in=annotation_type,
                        format_in=annotation_format,
                        save_path=path_to_svgs_temp
                    )

                    path_to_imgs_temp = os.path.join(img_folder, f'{i:09d}.png')
                    tile['image'].save(path_to_imgs_temp)
                ...
                if time.time() - prev_time > time_between_saves and len(results) != 0:
                    path_prediction = os.path.join(path_submission, f'predictions_{i:09d}.json')
    ...
    def post_process(self, post_process_config, verbose = True): # TODO TEST

        from ..methode.utils_post_processing import post_process_results
        raw_detections = self._load_all_detections()



        selected_detections = post_process_results(
            raw_detections,
            verbose = verbose,
            **post_process_config
        )

        with open(os.path.join(self.folder_results, "selected_detections.json"), "w") as file:
            json.dump(selected_detections, file, indent=4)
