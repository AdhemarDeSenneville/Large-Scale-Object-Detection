...
class OrthoCropApi:
    ...
    def get_available_products(self, *args, **kwargs):
        return 'Not Needed'
    ...
    def get(self, *args, **kwargs):
        for attempt in range(self.num_retries):
            try:
                self.delay_sleep()
                available_products = self.get_available_products(*args, **kwargs)

                if available_products != 'Not Needed':
                    if len(available_products) == 0:
                        warnings.warn(
                            f"[ERROR] No available products"
                        )
                        return None
                    kwargs['available_products'] = available_products
